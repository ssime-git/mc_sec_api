...
# Fit the OneHotEncoder with a dummy array matching the structure of the categorical features
dummy_data = np.array([
    ['Male', 'Red', 'Pizza']
])
...
async def predict_sign(user: User):
    ...
    return {
        ...
        "encrypted_first_name": encrypted_first_name.decode(),
        "encrypted_last_name": encrypted_last_name.decode(),
        "encrypted_email": encrypted_email.decode()
    }

# Endpoint for pseudonymizing user data
...
# Endpoint for decrypting user data
...
# Command to launch the API when running the Python script
